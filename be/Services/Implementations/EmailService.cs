...
namespace BloodDonationAPI.Services.Implementations
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<bool> SendEmailAsync(string to, string subject, string body, bool isHtml = true)
        {
            try
            {
                var enabled = bool.Parse(_configuration["EmailSettings:Enabled"] ?? "true");
                if (!enabled)
                {
                    _logger.LogInformation("📧 [EMAIL DISABLED] Would send email to {To} with subject: {Subject}", to, subject);
                    return true;
                }
