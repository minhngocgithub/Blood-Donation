...
namespace Blood_Donation_Website.Controllers
{
    ...
    public class DonationHistoryController : Controller
    {
        ...
        // GET: /DonationHistory/Index - Tìm kiếm lịch sử hiến máu (Admin/Hospital/Doctor/Staff)
        [AuthenticatedUser]
        public async Task<IActionResult> Index(string? searchTerm)
        {
            ...
            {
                // Tìm kiếm theo email, tên, mã đăng ký
                donations = await _donationHistoryService.SearchDonationsAsync(searchTerm);
                ViewBag.SearchTerm = searchTerm;
            }
            else
            {
                // Hiển thị tất cả (mặc định lấy 50 gần nhất)
                var allDonations = await _donationHistoryService.GetAllDonationsAsync();
                donations = allDonations.OrderByDescending(d => d.DonationDate).Take(50);
            }
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            // User chỉ xem được của mình, Staff/Doctor/Admin xem được tất cả
            var isStaffOrAbove = User.IsInRole("Admin") || User.IsInRole("Hospital") || User.IsInRole("Doctor") || User.IsInRole("Staff");
            if (!isStaffOrAbove && donation.UserId != userId)
            {
                TempData["Error"] = "Bạn không có quyền xem lịch sử hiến máu này.";
                return RedirectToAction(nameof(MyHistory));
            }
