    ...
    public class DonationRegistrationController : Controller
    {
        ...
        public async Task<IActionResult> MyRegistrations(string status = "active")
        {
            try
            {
                ...
                {
                    var activeStatuses = new RegistrationStatus[] { 
        ...
        public async Task<IActionResult> Cancel(int id, string? reason = null)
        {
            try
            {
                ...
                // Cho phép hủy ở các trạng thái: Registered, Confirmed, Deferred
                ...
                if (!cancellableStatuses.Contains(registration.Status))
                {
                    if (Request.Headers["X-Requested-With"] == "XMLHttpRequest")
                    {
                        return Json(new { success = false, message = "Đăng ký này không thể hủy. Chỉ có thể hủy đăng ký ở trạng thái 'Đã đăng ký', 'Đã xác nhận' hoặc 'Tạm hoãn'." });
                    }
                    TempData["Error"] = "Đăng ký này không thể hủy. Chỉ có thể hủy đăng ký ở trạng thái 'Đã đăng ký', 'Đã xác nhận' hoặc 'Tạm hoãn'.";
                    return RedirectToAction(nameof(Details), new { id });
                }

                // Sử dụng DisqualificationReason.Other cho lý do hủy tự do
                var reasonEnum = DisqualificationReason.Other;
